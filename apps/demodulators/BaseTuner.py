...
class BaseTuner(gr.hier_block2):
    ...
    def __init__(self, audio_rate: int, audio_bps: int, min_recording: float, classify: Classifier | None, notify_scanner: Callable) -> None:
        BaseTuner.channel += 1
    ...
    def _persist_wavfile(self, rf_center_freq: int) -> ChannelMessage | None:
        ...
        bytes_per_sample = self.audio_bps / 8
        min_size = 44 + (bytes_per_sample * self.audio_rate * self.min_recording)
